...
final as (
    select
        e.ees_id,
        b.derniere_depeche,
        case when a.ees_id is null then 0 else 1 end avec_ano,
        case when b.bloc_ees_id is null then 0 else 1 end avec_bloc,
        case when g.ees_id is null then 0 else 1 end avec_generic,
        case when f.ees_id is null then 0 else 1 end avec_fonction,
        case when m.ees_id is null then 0 else 1 end avec_equip,
        case when r.ees_id is null then 0 else 1 end avec_regle,
        case when e.ees_id is null or upper(e.ees_libl) like '%NPU%' or e.ees_libl like '*%' or upper(ees_libl) like '%NE PLUS UTIL%'
        then 1
        else 0
        end as ees_npu,
        b.nb nb_depeche,
        ...
        f.nb nb_fonction,
        m.nb nb_mots_clefs,
        case when ee.id_sagai is null then 1 else 0 end ees_non_repris
    from v_ees e	
